...
class HandSegmentation:
    ...
    def visualize_pcd(self,source, target, title):
        source.paint_uniform_color([1, 0, 0])   # Red for source (reference)
        target.paint_uniform_color([0, 1, 0])     # Green for target (to be moved)
    
        o3d.visualization.draw([
            {"name": "source", "geometry": source},
            {"name": "target_transformed", "geometry": target},
            {"name": "Coordinate Frame", "geometry": o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.5)}
        ], show_ui=True, show_skybox=False, title=title)
    ...
    def extract_filtered_points(self, mask, range_image, pcl_msg):
        ...
        try:
            ...
            if not original_cloud_filtered.empty:
                ...
                self.visualize_pcd(original_cloud, original_segmented_cloud, "Original Cloud and Segmented Cloud")

                # # Define the save location and ensure the directory exists
                # file_path = os.path.expanduser("~/mannequin_detection/extracted_clouds/black_jacket/filtered_chest_points.ply")
